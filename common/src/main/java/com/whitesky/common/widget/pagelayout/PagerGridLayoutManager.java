...
/**
 * @author xiaoxuan 2020-05-26 1. 网格布局 2. 支持水平分页和垂直分页 3. 杜绝高内存占用
 */
    ...
    implements RecyclerView.SmoothScroller.ScrollVectorProvider
{
    ...
    private float mCellAspectRatio = 0f;
    
    private SparseArray<GridCell> mCells;
    
    private List<Integer> mPagesNumber = new ArrayList<>(); // 每页的条目数
    ...
    private static class GridCell
    {
        ...
        {
            return "GridCell{" + "row=" + row + ", rowSpan=" + rowSpan + ", column=" + column + ", columnSpan="
                + columnSpan + '}';
        }
    }
     ...
     */
    public PagerGridLayoutManager(@IntRange(from = 1, to = 100) int rows, @IntRange(from = 1, to = 100) int columns,
     ...
     * 构造函数
     *
     * @param spanLookup 回调函数
     * @param rows 行数
     * @param columns 列数
     * @param orientation 方向
     */
    public PagerGridLayoutManager(GridSpanLookup spanLookup, int rows, int columns, int orientation)
    {
        mItemFrames = new SparseArray<>();
    ...
    private void calculateWindowSize()
    {
        ...
        {
            mCellAspectRatio = mItemWidth / mItemHeight;
            calculateItemCells();
        }
    }
    
    private void calculateItemCells()
    {
        mCells = new SparseArray<>(getItemCount());
        mPagesNumber = new ArrayList<>();
        int count = 0, everyNum = 0;// 计算最小单元数，每一页的个数
        if (getItemCount() <= 0)
            return;
        List<Point> locate = getLocationList();
        for (int itemPosition = 0; itemPosition < getItemCount(); itemPosition++)
        {
            everyNum++;
            SpanInfo spanInfo = mSpanLookup.getSpanInfo(itemPosition);
            Point first = locate.get(0);
            mCells.put(itemPosition, new GridCell(first.x, spanInfo.rowSpan, first.y, spanInfo.columnSpan));
            if (spanInfo.rowSpan == 1 && spanInfo.columnSpan == 1)
            {
                locate.remove(0);
            }
            else
            {
                for (int i = 0; i < spanInfo.rowSpan; i++)
                {
                    for (int j = 0; j < spanInfo.columnSpan; j++)
                    {
                        removePoint(locate, new Point(first.x + i, first.y + j));
                    }
                }
            }
            count += spanInfo.rowSpan * spanInfo.columnSpan;
            if (count == mOnePageSize)
            {
                mPagesNumber.add(everyNum);
                everyNum = 0;
                count = 0;
                locate = getLocationList();
            }
        }
        if (everyNum > 0)
        {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state)
    {
        ...
        {
            mMaxScrollX = (pageCount - 1) * getUsableWidth();
            mMaxScrollY = 0;
        ...
        {
            mMaxScrollX = 0;
            mMaxScrollY = (pageCount - 1) * getUsableHeight();
        ...
        {
            int twoPageNum = 0;
            if (mPagesNumber.size() == 1)
            {
                twoPageNum = mPagesNumber.get(0);
            }
            if (mPagesNumber.size() > 1)
            {
                twoPageNum = mPagesNumber.get(0) + mPagesNumber.get(1);
            }
            for (int i = 0; i < twoPageNum; i++)
            {
        ...
        if (mOffsetX == 0 && mOffsetY == 0)
        {
            ...
            if (mCellAspectRatio == 0)
            {
                onePageCount = mOnePageSize;
            }
            else
            {
                onePageCount = mPagesNumber.get(0);
            }
    ...
    private void recycleAndFillItems(RecyclerView.Recycler recycler, RecyclerView.State state, boolean isStart)
    {
        ...
        if (mCellAspectRatio > 0)
        {
            ...
            else
            {
                ...
                {
                    startPos += mPagesNumber.get(i);
                }
            }
            if ((pageIndex + 4) > (mPagesNumber.size() - 1))
            {
                stopPos = getItemCount();
            }
            else
            {
                stopPos = startPos + mPagesNumber.get(pageIndex) + mPagesNumber.get(pageIndex + 1)
                    + mPagesNumber.get(pageIndex + 2) + mPagesNumber.get(pageIndex + 3);
            }
        }
        else
        {
            ...
            stopPos = startPos + mOnePageSize * 4;
    ...
    private void addOrRemove(RecyclerView.Recycler recycler, Rect displayRect, int i)
    {
        ...
        {
            addView(child);
            measureChildWithMargins(child, mItemWidth, mItemHeight);
    ...
    private Rect getItemFrameByPosition(int pos)
    {
        ...
        if (null == rect)
        {
            ...
            {
                GridCell cell = mCells.get(pos);
                offsetX += cell.column * mItemWidth;
                offsetY += cell.row * mItemHeight;
                rect.left = offsetX;
                rect.top = offsetY;
                rect.right = offsetX + cell.columnSpan * mItemWidth;
                rect.bottom = offsetY + cell.rowSpan * mItemHeight;
            }
    ...
    private List<Point> removePoint(List<Point> list, Point p)
    {
    ...
    public int getTotalPageCount()
    {
        ...
        if (mCellAspectRatio > 0)
        {
            totalCount = mPagesNumber.size();
        }
        else
        {
            totalCount = getItemCount() / mOnePageSize;
    ...
    /**
     * 根据页面，找到当前页第一个item的position
     *
     */
    private int getFirstPosByPageIndex(int pageIndex)
    {
        int position = 0;
        if (mCellAspectRatio == 0)
        {
            position = pageIndex * mOnePageSize;
        }
        else
        {
            if (pageIndex == 0)
    ...
    private int getPageIndexByPos(int pos)
    {
        ...
        {
            int total = 0;
            for (int position = 0; position < mPagesNumber.size(); position++)
            {
                total += mPagesNumber.get(position);
                if (pos < total)
                {
                    pageIndex = position;
                    break;
    ...
    public int findLastVisibleItemPosition()
    {
        ...
        page++;
        int position = (page + 1) * mOnePageSize;
        return position - 1;
    }
    ...
    int findNextPageFirstPos()
    {
        ...
        return getFirstPosByPageIndex(page);
    }
    ...
    /**
     * 获取偏移量(为PagerGridSnapHelper准备) 用于分页滚动，确定需要滚动的距离。 {@link PagerGridSnapHelper}
     *
    ...
    public View findSnapView()
    {
        ...
        int targetPos = getFirstPosByPageIndex(getPageIndexByOffset()); // 目标Pos
    ...
    public void smoothScrollToPage(int pageIndex)
    {
        ...
        int position = getFirstPosByPageIndex(pageIndex);
